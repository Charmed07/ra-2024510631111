...
> **Nomes:** João Antonio   **Turma:** ____________________   **Data:** 17 /09 / 2026
...
1. Qual é a falha? Inject 

2. Qual o dano possível em produção? O dado ele vem como codigo.

3. Como corrigir? _db.Clientes.FromSqlInterpolated(
...
1. Qual é a falha? Broken Access Control 
var fatura = _db.Faturas.Find(id);
       if (fatura == null) return NotFound();

2. Qual o dano possível em produção? Pegar os dados de outro cliente
...
Misconfiguration e secrets no codigo
...
Acesso a senhas que fica gravado no historico
...
1. Qual é a falha? Mass assignment
public string Role  { get; set; }   // "user" | "admin"

2. Qual o dano possível em produção? 

A confiança no cliente em questão de acesso 
