...
Tempo primeiro teste 4.33s
Segundo teste : 4.04 S
Terceiro teste: 4.03 s
Quarto teste: 4.4 s
...
Sim , processou os 5 na sequencia.
- Alguma notificação falhou? O que aconteceu? (a resposta do pedido mudou?)
Sim , uma notificação falhou, mas a resposta foi enviado ao postman.
...
O pagamento foi aprovado e deu erro ao notificar , no postman deu erro de 500 Internal Server Error
- **Assíncrono** (`/api/pedidos/assincrono`) → O que acontece?
O pagamento foi aprovado , notificação pendente , a resposta no postman foi de 202 Accepted
...
Não, o pedido rejeitado não gera notificação em nenhum dos dois fluxos. Porque o código executa um return BadRequest()assim que o pagamento é recusado. Isso encerra a requisição imediatamente, impedindo que o sistema alcance as linhas de código que enviariam a notificação ou a publicariam na fila.
---
...
O pagamento é mantido síncrono porque a regra de negócios exige uma resposta imediata e definitiva antes de confirmar a compra. Poderia ser assíncrono (como em pagamentos via Pix ou boleto), mas isso mudaria o design da API para trabalhar com consistência eventual: o pedido nasceria com o status "pendente" e só seria faturado em um momento futuro, após o serviço de pagamento avisar a loja
...
Como o Channel opera exclusivamente na memória RAM da aplicação, qualquer reinício de serviço, queda ou falha no servidor fará com que todos os eventos pendentes na fila sejam perdidos permanentemente. O cliente seria cobrado, mas o sistema esqueceria de enviar o e-mail.

3. **Em produção, o que substituiria o `Channel<T>`?** Cite pelo menos uma tecnologia.
O uso de filas em memória seria substituído por um Message Broker profissional, que grava as mensagens fisicamente em disco para garantir que sobrevivam a reinicializações. As tecnologias mais utilizadas incluem RabbitMQ, Apache Kafka ou provedores em nuvem como Amazon SQS e Azure Service Bus.
...
O consumidor deve fazer tentativas de reenvio com intervalos de tempo crescentes (Retry with backoff). Se esgotar o limite de tentativas e o erro persistir, a mensagem deve ser movida para uma fila de erros (Dead-Letter Queue - DLQ) para análise manual, evitando que ela fique em loop e trave o processamento dos próximos pedidos.

5. **Compare os tempos de resposta** que você anotou:

| Fluxo | Tempo de resposta | Notificação chega? |
|-------|-------------------|--------------------|
| Síncrono | ~4.33 ms |  Imediata Sim  / Pode falhar Sim |
| Assíncrono | ~1.27 ms | Em background Sim /  Garantida?Não |
